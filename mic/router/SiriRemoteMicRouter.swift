...
//
//  Tails PacketLogger's binary capture, extracts A2854 GATT voice notifications, decodes Opus,
//  and feeds 48 kHz mono audio to the HAL plug-in's stereo shared ring.
//
import Darwin
import Foundation
...
private final class Router {
    ...
    private var sequenceTracker = VoiceSequenceTracker()
    ...
    func consume(_ frame: RemoteVoiceFrame) throws {
        ...
        if observedVoiceStreams.insert(stream).inserted, !options.quiet {
            fputs("SiriRemoteAudioRouter: voice stream \(stream)\n", stderr)
        }
        ...
        case .discontinuity:
            // A new Siri hold normally restarts the sequence at zero. Do not synthesize a huge
            // gap; the next real packet establishes the new stream.
    ...
    func runPklg(path: String) throws {
        ...
        var bytesRead = 0
        var lastDiagnostic = Date.timeIntervalSinceReferenceDate

        func reportProgress(force: Bool = false) {
            guard !options.quiet else { return }
            let now = Date.timeIntervalSinceReferenceDate
            guard force || now - lastDiagnostic >= 1.0 else { return }
            lastDiagnostic = now
            let rms = stats.decodedSamples == 0
                ? 0.0
                : (stats.sumSquares / Double(stats.decodedSamples)).squareRoot()
            fputs(
                "SiriRemoteAudioRouter: pklg bytes=\(bytesRead) "
                    + "records=\(extractor.recordsScanned) acl=\(extractor.receivedACLRecords) "
                    + "l2cap=\(extractor.completedL2CAPPackets) "
                    + "voice=\(extractor.extractedVoiceFrames) decoded=\(stats.decodedFrames) "
                    + "bad=\(stats.decodeErrors) rms=\(String(format: "%.1f", rms)) "
                    + "peak=\(stats.peak) ring=\(srm_ring_writer_write_index())\n",
                stderr
            )
        }

        if !options.quiet {
            fputs("SiriRemoteAudioRouter: tailing fresh capture \(path)\n", stderr)
        }
        ...
        while true {
            ...
            if n > 0 {
                ...
                if options.exitOnEOF { reportProgress(force: true); break }
                if gInterrupted != 0 { reportProgress(force: true); break }
                reportProgress()
...
private func printUsage() {
    print("""
    Usage:
      SiriRemoteAudioRouter [--input TRACE.txt | --pklg FILE.pklg]
    ...
    static func main() {
        do {
            ...
            if let path = options.pklgPath {
                ...
                contents.enumerateLines { line, _ in
                    do {
                        ...
                        fputs("SiriRemoteAudioRouter: \(error)\n", stderr)
                        exit(1)
                    }
                }
            } else {
            ...
            let rms = stats.decodedSamples == 0
                ...
                : (stats.sumSquares / Double(stats.decodedSamples)).squareRoot()
            if !options.quiet {
                print("SiriRemoteAudioRouter: lines=\(stats.inputLines) voice=\(stats.voiceFrames) "
                ...
                print(String(format: "SiriRemoteAudioRouter: samples=%d rms=%.1f peak=%d ring_write=%llu",

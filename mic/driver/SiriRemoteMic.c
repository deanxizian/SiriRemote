...
// A new first-client StartIO resets sample time to zero and therefore starts a new CoreAudio
// timeline. The seed must change with that discontinuity; otherwise HAL can interpret the reset as
// the old timeline moving backwards and repeatedly reschedule work in coreaudiod.
static UInt64                       gDevice_ZeroTimeStampSeed           = 0;
...
// shared-memory ring — see SiriRemoteMicShared.h. The only source is decoded A2854 voice.
// When that producer is absent or inactive the device returns silence; it never falls back to
// the Mac's built-in microphone. The producer creates the region; we attach once on StartIO and
// read lock-free from the real-time
...
// offset) that stays FIXED while primed; re-reads are idempotent, and the offset
...
#define kSRM_PreRollFrames          24000u    // preserve 500 ms while a new input source settles
...
// observe producer progress; later reads must return the exact same audio. A single-window cache
// covers the host's consecutive per-client reads without allocation or locking on the RT path.
#define kSRM_RenderCacheFrames      8192u
#define kSRM_CacheInvalidTime       (INT64_MIN / 4)
...
static void SRM_AttachRing(SRMRingReader* r)
{
    ...
    void* p = mmap(NULL, sizeof(SRMSharedMemory), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);
    if (p == MAP_FAILED)
    {
    ...
    r->offset = 0;
    r->generation = atomic_load_explicit(&m->generation, memory_order_acquire);
    r->shm = m;
...
static void SRM_ResetReaderSession(SRMRingReader* r)
{
    ...
    r->gate = r->primeFrames;
    r->offset = 0;
    const uint64_t currentWrite = (r->shm != NULL)
        ? atomic_load_explicit(&r->shm->writeIndex, memory_order_acquire)
        : 0;
    r->consumedTo = currentWrite > kSRM_PreRollFrames
        ? currentWrite - kSRM_PreRollFrames
        : 0;
    r->generation = (r->shm != NULL)
        ? atomic_load_explicit(&r->shm->generation, memory_order_acquire)
        : 0;
    if (r->shm != NULL)
    {
        atomic_store_explicit(&r->shm->readIndex, r->consumedTo, memory_order_release);
        atomic_store_explicit(&r->shm->playbackStartFrame, r->consumedTo,
...
static void SRM_ResetSession(void)
{
    ...
    gSRM_RenderCacheTime = kSRM_CacheInvalidTime;
...
// preserving up to 500 ms of pre-roll captured before the consumer opened the device.
static uint64_t SRM_RingObserve(SRMRingReader* r, int64_t inNow)
{
    ...
    if (generation != r->generation || w < r->consumedTo)
    {
        ...
        r->primed = false;
        r->gate = r->primeFrames;
        r->consumedTo = w > kSRM_PreRollFrames ? w - kSRM_PreRollFrames : 0;
        atomic_store_explicit(&r->shm->readIndex, r->consumedTo, memory_order_release);
        atomic_store_explicit(&r->shm->playbackStartFrame, r->consumedTo,
                              memory_order_release);
    }
...
// ------------------------------------------------------------------------------------------------
#define kSRM_ConsumersNotification  "com.deanxi.siriremote.audio.consumers"
...
static void SRM_PublishSharedConsumerState(UInt64 inTotalRunning, Boolean inFirstClient)
{
    if (gSRM_Remote.shm == NULL) { return; }
    atomic_store_explicit(&gSRM_Remote.shm->consumerCount,
                          (uint32_t)inTotalRunning, memory_order_release);
    if (inFirstClient)
    {
...
static OSStatus	BlackHole_StartIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID)
{
    ...
    {
        // GetZeroTimeStamp reads these fields under gDevice_IOMutex. Reset them under the same
        // lock so the audio host can never observe a half-old, half-new timeline during churn.
        ...
        ++gDevice_ZeroTimeStampSeed;
        if (gDevice_ZeroTimeStampSeed == 0) ++gDevice_ZeroTimeStampSeed;
    ...
    SRM_PublishSharedConsumerState(gDevice_IOIsRunning + gDevice2_IOIsRunning,
                                   (gDevice_IOIsRunning + gDevice2_IOIsRunning) == 1);

    // Publish device-use telemetry for drain checks and diagnostics. This does not start Bluetooth
    // capture: input methods may keep the virtual device open while no Siri-button session exists.
    // Still under the state mutex so counter changes and notifications cannot arrive out of order.
...
#endif
    SRM_PublishSharedConsumerState(gDevice_IOIsRunning + gDevice2_IOIsRunning, false);

    // Mirror the StartIO telemetry when a client stops. Capture lifetime is independently owned by
    // the App's live-PID voice lease, not by this count.
    SRM_PublishConsumerCount(gDevice_IOIsRunning + gDevice2_IOIsRunning,
...
static OSStatus	BlackHole_GetZeroTimeStamp(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, Float64* outSampleTime, UInt64* outHostTime, UInt64* outSeed)
{
	...
	//	Move to the newest whole zero-timestamp period. A suspended or delayed audio host can miss
	//	more than one period; advancing only one per call makes it busy-loop to catch up.
	if(theNextHostTime <= theCurrentHostTime)
	{
		UInt64 thePeriodsElapsed = 1;
		if(theAdjustedTicksPerRingBuffer > 0.0)
		{
			thePeriodsElapsed += (UInt64)(((Float64)(theCurrentHostTime - theNextHostTime)) /
			                                  theAdjustedTicksPerRingBuffer);
		}
		gDevice_NumberTimeStamps += thePeriodsElapsed;
		gDevice_PreviousTicks += ((Float64)thePeriodsElapsed) * theAdjustedTicksPerRingBuffer;
	}
...
static OSStatus	BlackHole_DoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer)
{
    ...
    // Router → Siri Remote Mic. This product has one producer ring only; without a valid active
    // A2854 producer the input is silence. The RT path performs atomics, arithmetic and memcpy.
    if(inOperationID == kAudioServerPlugInIOOperationReadInput)
    {
        ...
        if (remoteUsable)
        {
            ...
            (void)SRM_RingRender(&gSRM_Remote, remoteWrite, now,
                                 inIOBufferFrameSize, outMain);
